...
public class ProductListings extends AppCompatActivity {
    ...
    String curr_filter, from, ItemIds[];
    CardView filterlayout;
    LinearLayout material, categ, room, style, color, SelectedFiltersLayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent i = getIntent();
        Log.d("product listing", "listing");
        SubId = i.getIntExtra("subId", 0);
        Log.d("product listing subid", String.valueOf(SubId));
        s = i.getStringExtra("subCat");
        from = i.getStringExtra("from");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle(s);
        sharedPreferences = getApplicationContext().getSharedPreferences("cart", Context.MODE_PRIVATE);
        Button addsample = (Button) findViewById(R.id.addsample);
        ...
        GridLayoutManager lLayout = new GridLayoutManager(ProductListings.this, 3);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                ...
                return true;
            case R.id.cart_menu:
                Intent i = new Intent(ProductListings.this, Cart.class);
                startActivity(i);
        }
    ...
    private void GetSubCatItems(final int subid) {
        final RequestQueue rq = VolleySingelton.getInstance(getApplicationContext()).getRequestQ();

        StringRequest sr = new StringRequest(Request.Method.POST, "http://www.woodndecor.in/items.php", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                        ...
                        for (int i = 0; i < arrJson.length(); i++) {
                            ...
                            switch (filte[i]) {
                                case "category":
                                    ...
                                    arrx = jsonObject.getJSONArray("categoryid_options");
                                    filter.categ_id = new String[arrx.length()];
                                    for (int x = 0; x < arrx.length(); x++) {
                                        filter.categ_id[x] = arrx.getString(x);
                                    }
                    ...
                    if (from.equals("home")) {
                        adapter = new GalleryAdapter(getApplicationContext(), fromJson(arr, ItemIds, firstImg), false, true, "home", subid);
                    } else if (from.equals("shop")) {
                        adapter = new GalleryAdapter(getApplicationContext(), fromJson(arr, ItemIds, firstImg), false, true, "shop", subid);
                    }
            ...
            protected Map<String, String> getParams() {
                ...
                if (filter.catid_opt != null) {
                    params.put("subid", String.valueOf(filter.catid_opt));
                } else {
                    params.put("subid", String.valueOf(subid));
                }
                return params;
            }
        };
    ...
    private List<GalleryItemObject> fromJson(String[] arr, String[] iudA, String[] img) {


        for (int i = 0; i < arr.length; i++)
            allItems.add(new GalleryItemObject(arr[i], img[i], Integer.parseInt(iudA[i]), "yes"));
        return allItems;
    ...
    public void filter_selected(View v) {
        ...
        switch (curr_filter) {
            case "Category":
                ...
                for (int i = 0; i < filter.categ.length; i++) {
                    if (filter.categ[i].equals(filter.categ_opt)) {
                        filter.catid_opt = filter.categ_id[i];
                    }
                }
                break;
            case "Style":
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (arrfilter[0].equals(curfilter)) {
                if (arrfilter[1].equals(v.getTag())) {
                    ...
                    Toast.makeText(ProductListings.this, "this filter is already added", Toast.LENGTH_SHORT).show();
                    z = 1;
                } else {
    ...
    public void resetfilter(View v) {
        ...
        adapter.notifyDataSetChanged();
        avcHiomne.setVisibility(View.VISIBLE);
        GetSubCatItems(SubId);        //load items with no filters
    }

    public void addsa() {
        Log.d("cart_items", "item");
        SharedPreferences.Editor editor = sharedPreferences.edit();
        int qty = 999;
        Log.d("cart_items", "item id which you are adding: " + SubId);
        ...
        //if (SubId >= 0) {
            if (!items.equals("0")) {
                ...
                /*if (Arrays.asList(cart_array).contains(String.valueOf(SubId))) {
                    ...
                    items += String.valueOf(SubId) + ":" + qty + "," ;
                ...
                int j=0,g=0;
                for (int z = 0; z < ItemIds.length; z++) {

                    if (Arrays.asList(cart_array).contains(String.valueOf(ItemIds[z]))) {
                        if(j==0) {
                            j++;
                            Log.d("cart_items", s + " is already in your cart!!");
                            Toast.makeText(this, s + " is already in your cart!!", Toast.LENGTH_SHORT).show(); }
                    } else {
                        items += String.valueOf(ItemIds[z]) + ":" + qty + ",";
                        //Toast.makeText(this, "Item " + arr[z] + " Added in your cart!!", Toast.LENGTH_SHORT).show();
                       if(g==0) {
                           g++;
                           Log.d("cart_items", "saving array: " + items);
                           Toast.makeText(this, s + " added in your Cart!", Toast.LENGTH_SHORT).show();}
                        editor.putString("cart_item", items);
                        editor.commit();
                    }
                ...
                Log.d("cart_items","there were no items in your cart");
                Toast.makeText(this, "Item Added in your cart!!", Toast.LENGTH_SHORT).show();
                editor.putString( "cart_item", String.valueOf(SubId) + ":" + qty + "," ) ;
                //Toast.makeText(this, "Item Added in your cart!!", Toast.LENGTH_SHORT).show();*/
                int f = 0,j=0,g=0;
                Log.d("cart_items","No. of items to add: " + ItemIds.length);
                for (int zx=0; zx < ItemIds.length; zx++) {
                    Log.d("cart_items", "for iteration number: " + zx);
                    if (f == 0) {
                        f++;
                        Log.d("cart_items", "adding item: " + ItemIds[zx]);
                        items = String.valueOf(ItemIds[zx]) + ":" + qty + ",";
                        editor.putString("cart_item", String.valueOf(ItemIds[zx]) + ":" + qty + ",");
                        editor.commit();
                        if(j==0) {
                            j++;
                            Toast.makeText(this, s + " is added in your cart!!", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        items += String.valueOf(ItemIds[zx]) + ":" + qty + ",";
                        Log.d("cart_items", "adding Remaining items: " + ItemIds[zx]);
                        editor.putString("cart_item", items);
                        editor.commit();
                        if(g==0) {
                            g++;
        ...
        Log.d("cart", "after saving array: " + sharedPreferences.getString("cart_item", "null"));
